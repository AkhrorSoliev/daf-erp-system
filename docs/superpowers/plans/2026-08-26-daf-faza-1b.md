...
Bu reja bajarildi va yakuniy butun-shox ko'rigidan o'tdi (2 Critical, 9 Important
topildi va bitta tuzatish to'lqinida yopildi). Quyidagilar **ataylab bu shoxda
hal qilinmadi**:

**Adapter interfeysi hali bitta manba tomonidan qo'llanadi.** Spec 5-bo'limi
uchta adapter (`dib`, `wort-schule`, `zum`) shu interfeysga qurilishini talab
qilgan; amalda faqat `WortSchuleAdapter` uni amalga oshiradi. `daf-harvest.ts`
DiB'ni hamon to'g'ridan-to'g'ri `DibClient` va erkin parser funksiyalari orqali
boshqaradi, `assets()` metodining esa prodaksiyada chaqiruvchisi yo'q — haqiqiy
aktiv yo'li `collectAssets(dataset)`. R6 ning asosiy maqsadi (kesh kalitining
ajralib ketishi) `CachedHttpClient` bilan yopildi; R7 esa yopilmadi. **Bu
sizning qaroringiz**: interfeysni Faza 2 da to'ldirish, yoki uni butunlay
qayta ko'rib chiqish.

**Javob kaliti saqlanishi hal qilinmagan.** 1 049 mashqdan 67 tasi bittadan
ortiq javob talab qiladi (61 GAP 2–5 bo'sh joyli, 2 MC, 3 CLOZE 10–13).
`answer: string | null` bularni ushlay olmaydi. Endi har mashqda `blankCount`
bor, ya'ni Faza 2 nechta javob kerakligini ma'lumotdan biladi — lekin
javoblarni qanday saqlash Faza 2 ning modeli bilan birga hal qilinadi.

**`GRAMMAR_LEVEL` 92 sahifadan 46 tasini qoplaydi.** Qolgani `level: null`.
Xarita taxmin qilmaslikni ataylab tanlagan; Faza 2 daraja bo'yicha filtrlashni
qurganda bu bo'shliq ko'rinadi.

**Lint darvozasi ko'ringanidan zaifroq.** `server/package.json` dagi `lint`
skripti `scripts/` katalogini qamramaydi (ya'ni `daf-harvest.ts` va
`daf-upload-media.ts` hech qachon tekshirilmaydi) va `--fix` bilan ishlagani
uchun tuzatib, 0 kod bilan chiqadi — yiqilmaydi.

**404 aniqlash hamon xato xabarining matniga tayanadi.** `HttpStatusError`
kabi tipli xatoga o'tish Faza 2 ning ishi.
