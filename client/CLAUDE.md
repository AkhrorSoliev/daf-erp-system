...
- **`salary-center-topup-dialog.tsx`** — opens from the "Markaz qo'shimchasi" card by clicking either **Qolgan (markaz)** or **O'quvchilardan olinishi kerak** (both are `DrillDownAmount` buttons — dotted underline + `ArrowUpRight`). Its body lives in `debt/center-topup-content.tsx` and is shared with the `/payments/debt` "Markaz qoplagani" tab; this file is only the dialog chrome. Lists the students the center is still owed by, with exactly TWO money columns: **Markaz ustozga to'lagan** (the center's outlay this month) and **O'quvchining qarzi** (their debt today — the same figure as their profile, because that is what an admin asks for on the phone). Two other versions of that second column were tried and removed: what the lessons cost (#10026 showed 345 000 against a real debt of 156 000) and `min(debt, lesson cost)` (#10058 showed 466 662 against a profile saying 624 989). Students with nothing left to collect are never listed and there is no toggle to show them; the sub-line under the header reconciles the gap to the card ("kartada X — farqi puli qaytgan N o'quvchi"). The only filter is student status. Every figure on screen follows that filter, headline sums included.
