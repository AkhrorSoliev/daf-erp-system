...
- **`salary-center-topup-dialog.tsx`** — opens from the "Markaz qo'shimchasi" card by clicking either **Qolgan (markaz)** or **O'quvchilardan olinishi kerak** (both are `DrillDownAmount` buttons — dotted underline + `ArrowUpRight`). Lists the students the center is still owed by, with exactly TWO money columns: **Markaz ustozga to'lagan** (the center's outlay) and **Shu oydan qolgan qarzi** (`min(debt today, this month's fronted lessons)`). A third column showing what those lessons cost was removed — it was a historical figure that read as a demand, and #10026 displayed 345 000 there against a real debt of 156 000. Students with nothing left to collect are never listed and there is no toggle to show them; the sub-line under the header reconciles the gap to the card ("kartada X — farqi puli qaytgan N o'quvchi"). The only filter is student status. Every figure on screen follows that filter, headline sums included.
