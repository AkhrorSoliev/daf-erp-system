...
#### Legal pages and the company identity

- **`src/lib/company.ts` is the only place the company names itself.** `COMPANY.legalName` («DAF SPRACHZENTRUM» MChJ) belongs on anything with legal weight — the login footers' copyright line and the privacy policy; `COMPANY.tradingName` is for ordinary UI. `address` and `taxId` are `null` until the registry values are filled in, and every consumer renders them only when set, so a blank never prints as a placeholder that reads like a real value.
- The copyright line in `LoginFooter` names the legal entity, not the portal, on every portal. It links to `/privacy` and to Telegram support. `PortalConfig.footerText` was removed; do not re-add a per-portal copyright string.
- **Store badges (`showAppLinks`) are student-only.** `LoginFooter` renders `<StoreBadges />` only when the prop is passed, and only the student branch passes it — the mobile app is the *student* app, so on the admin and teacher logins the badges would invite someone to install something that does not exist for them.
- **The badges live on the footer's existing row, not a row of their own.** A dedicated row above the copyright line took the strip from 41px to ~107px, which is a lot of a login page spent on something that is not signing in. Text sits left, badges right, and `BADGE_HEIGHT` (26px) is deliberately small so the badges never drive the row's height — with them the footer is 64px. Do not enlarge the badges without re-checking that number.
- `components/auth/store-badges.tsx` uses the stores' **own artwork**, unmodified (`public/badges/`) — both companies' brand guidelines require their supplied asset and forbid recolouring or redrawing it, so do not swap in a hand-made SVG or a `currentColor` variant for dark mode. The black badges carry their own light rim and read correctly on both themes.
- **The two files are not sized the same way.** Apple's SVG is tight artwork (119.66×40); Google's PNG is a 646×250 canvas whose outer third is the clear space its guidelines mandate, so only 67.2% of the file's height is the badge. `STORES[*].visibleHeightRatio` carries that number and `StoreBadges` divides `BADGE_HEIGHT` by it — which is what makes the two drawn badges the same height. Do not "fix" the apparent size difference by cropping the Google file.
- **`/privacy` is public on every host** — excepted in `middleware.ts` twice: once in the public-portal short-circuit (or `invoice.`/`form.` would rewrite it to `/r/privacy`) and once before the unauthenticated redirect. It is linked from the login footers and from the app store listings, so it must open with no session anywhere. Exact match only.
- The policy text lives as data in `components/legal/privacy-content.ts`, rendered by `privacy-policy.tsx` in either skin (Lumio on `student.`, shadcn elsewhere). **When a data flow changes — a new payment provider, notification channel, or field on the student record — update the policy in the same PR and move `LAST_UPDATED`.**
...
- **Admin and student logins share this treatment; the teacher portal does not.** Both branches of `app/(auth)/login/page.tsx` render a full-bleed photo through `next/image` (`fill priority sizes="100vw"`) with a scrim over it, and put the form on a `.liquid-glass` pane (`w-full max-w-sm rounded-[28px] p-6 sm:p-8`) with the footer on a `bg-background/85 backdrop-blur-sm` strip. Admin uses `/login-admin-background.jpg`; student uses `/login-student-background.jpg`.
- **The two scrims are deliberately different, and the student's must not be lowered to match admin's.** The student photo is a bright above-the-clouds daylight sky, so `bg-slate-950/15 dark:bg-slate-950/70` — barely tinted in light, heavy in dark, because the dark glass is translucent and a bright photo shows straight through it and strands `--ink-900` (near-white in dark) on a white cloud. Admin's darker photo needs `bg-slate-950/20 dark:bg-slate-950/55`.
- The student branch keeps its `.lumio` wrapper: the pane's contents are Lumio primitives, and `.liquid-glass` is theme-aware on its own (`.dark .liquid-glass`), so no Lumio-specific override is needed.
...
- Layout, three layers (CEO, 26.09.2026: the first version showed everything at once and was hard to read at a glance): (1) header with "PDF yuklab olish" (`GET /students/:id/statement.pdf` as a blob), `view.warning` as a red banner, then `statement-summary.tsx` — the answer box and tiles To'lagan · Darslar narxi · Boshqa (only when non-zero) · Qoldiq from `model.equation`, which add up; (2) "Oylar bo'yicha" (`statement-months-table.tsx`), one line per month — the grey detail lines, `view.sharpNote` and the lesson-day chips open together on a row click (`MonthDetails`), the sharp-change row stays yellow; (3) "Batafsil": closed `StatementSection`s for "To'lovlar qayerga ketdi" (`statement-allocations.tsx`), "To'lov turi o'zgarishi", "Hisob qanday chiqdi" (the equation sentence and the footnote) and "Barcha yozuvlar". `statement-report.tsx` draws it without fetching (the ledger comes in as a prop), so `statement-report.test.ts` renders it statically. The months table is a whole-course summary and is deliberately not paginated.
...
- **No bolt-in-a-coral-square brand mark.** A `Lightning` glyph on a `rounded-3xl bg-coral-500 clay-coral` tile used to head the student login and the "Biz haqimizda" screen; both were removed, and it must not come back as a logo anywhere — the center has a real logo (`public/daf-logo.png`) and an invented mark competing with it is worse than no mark. Those two screens now lead with the heading alone. `Lightning` stays in `lumio/icon.tsx` and is fine as an ordinary icon; the ban is on using it *as the brand*.
