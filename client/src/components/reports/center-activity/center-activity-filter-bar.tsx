...
export interface CenterActivityFilter {
...
interface CenterActivityFilterBarProps {
  value: CenterActivityFilter;
  onChange: (next: CenterActivityFilter) => void;
}
...
export function resolveCenterActivityRange(f: CenterActivityFilter): {
  start: Date;
  end: Date;
  label: string;
} {
...
export function defaultCenterActivityFilter(): CenterActivityFilter {
  const now = new Date();
  return {
    branchId: null,
...
export function CenterActivityFilterBar({
  value,
  onChange,
}: CenterActivityFilterBarProps) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      ...
      <div className="ml-auto inline-flex items-center rounded-md border bg-card p-0.5">
        {BUCKETS.map((b) => (
          <Button
            key={b}
            type="button"
            variant={value.bucket === b ? "default" : "ghost"}
            size="sm"
            className="h-8 px-3 text-xs"
            onClick={() => onChange({ ...value, bucket: b })}
          >
            {BUCKET_LABELS[b]}
          </Button>
        ))}
