...
interface Props {
  ...
  period: { periodStart: string; periodEnd: string; cycleStartDay: number } | null;
  onChanged: () => void;
}
 ...
 * control verbatim.
 */
export function SalarySettingsSheet({ open, onOpenChange, period, onChanged }: Props) {
  ...
  const editingRow = rows.find((r) => r.user.id === editingUserId) ?? null;
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="flex w-full flex-col gap-0 p-0 sm:max-w-xl">
        <SheetHeader className="border-b px-6 py-4">
          ...
          <SheetDescription>
            Ustoz stavkalari va hisoblash davri. Jadval shu sozlamalar asosida
            hisoblanadi.
          </SheetDescription>
        </SheetHeader>
        ...
        <div className="flex-1 space-y-6 overflow-y-auto px-6 py-5">
          ...
          {period && (
            <section className="space-y-2">
              ...
              <SalaryPeriodControl period={period} isCeo onChanged={onChanged} />
            </section>
          )}
                ...
                rows.map((row) => (
                  ...
                  >
                    ...
                    <Button
                      ...
                      onClick={() => setEditingUserId(row.user.id)}
        ...
        {/* Per-teacher rate editor */}
        <SalaryConfigRowSheet
          userId={editingUserId}
          employee={
            editingRow
              ? {
                  id: editingRow.user.id,
                  firstName: editingRow.user.firstName,
                  lastName: editingRow.user.lastName,
                  branches: editingRow.user.branch ? [editingRow.user.branch] : [],
                  roles: [TEACHER_ROLE],
                }
              : null
          }
