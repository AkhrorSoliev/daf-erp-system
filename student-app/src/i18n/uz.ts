/**
 * App copy — O'zbekcha (Latin ONLY, never Cyrillic/Arabic letters).
 *
 * This file is the SOURCE OF TRUTH for the string shape. Every other language
 * file (de.ts, later en.ts) must satisfy `Dict` — if a key is missing there,
 * TypeScript fails the build, so no translation can silently drop out.
 *
 * Single source so copy is OTA-updatable via EAS Update.
 */
export const uz = {
  common: {
    loading: 'Yuklanmoqda...',
    retry: 'Qayta urinish',
    error: 'Xatolik yuz berdi',
    errorTitle: 'Xatolik',
    loadFailed: "Ma'lumotni yuklab bo'lmadi",
    save: 'Saqlash',
    cancel: 'Bekor qilish',
    ok: 'OK',
    back: 'Orqaga',
    comingSoon: 'Tez orada',
    showPassword: "Parolni ko'rsatish",
    hidePassword: 'Parolni yashirish',
  },
  // Bottom tab-bar labels.
  nav: {
    index: 'Asosiy',
    darslar: 'Darslar',
    resurslar: 'Resurslar',
    more: "Ko'proq",
  },
  // Stack / screen header titles.
  tabs: {
    home: 'Asosiy',
    schedule: 'Jadval',
    attendance: 'Davomat',
    payments: "To'lovlar",
    profile: 'Profil',
    more: "Ko'proq",
    settings: 'Sozlamalar',
    faq: 'FAQ',
    about: 'Biz haqimizda',
  },
  auth: {
    phoneLabel: 'Telefon raqami',
    phonePlaceholder: '90 123 45 67',
    passwordLabel: 'Parol',
    passwordPlaceholder: '••••••',
    continue: 'Davom etish',
    otpTitle: 'Tasdiqlash kodi',
    otpHint: 'Telegram bot yuborgan kodni kiriting',
    login: 'Kirish',
    forgot: 'Parolni unutdingizmi?',
    brand: 'DAF Student',
    tagline: "DAF Sprachzentrum — o'quvchi kabineti",
    loginErrorTitle: 'Kirish xatosi',
    orDivider: 'yoki',
    telegramLogin: 'Telegram orqali kirish',
    telegramConfirmTitle: "Telegram'da tasdiqlang",
    telegramHintBefore: 'Botda ',
    telegramHintWord: 'START',
    telegramHintAfter: ' tugmasini bosing — tasdiqlangach avtomatik kirasiz.',
    telegramReopen: 'Telegramni qayta ochish',
    timeoutTitle: 'Vaqt tugadi',
    timeoutMessage: "Qayta urinib ko'ring",
  },
  forgotPassword: {
    title: 'Parolni tiklash',
    step1Hint:
      'Telefon raqamingizni kiriting — SMS orqali tasdiqlash kodi yuboramiz.',
    step2Hint: 'Raqamingizga yuborilgan 4 xonali kodni kiriting.',
    step3Hint: "Yangi parolingizni o'rnating.",
    codeLabel: 'Tasdiqlash kodi',
    newPasswordLabel: 'Yangi parol',
    confirmPasswordLabel: 'Parolni takrorlang',
    newPasswordPlaceholder: 'Kamida 6 ta belgi',
    sendCode: 'Kod yuborish',
    verify: 'Tasdiqlash',
    save: 'Saqlash',
    resend: 'Qayta yuborish',
    changeNumber: "Raqamni o'zgartirish",
    successTitle: "Parol o'zgartirildi",
    successMessage: 'Endi yangi parol bilan kirishingiz mumkin.',
    errPhone: "Telefon raqamni to'liq kiriting",
    errCode: "Kod 4 xonali bo'lishi kerak",
    errPassword: "Parol kamida 6 ta belgidan iborat bo'lishi kerak",
    errMismatch: 'Parollar mos kelmadi',
  },
  home: {
    greeting: 'Assalomu alaykum',
    balance: 'Balans',
    inDebt: 'Qarzdorlik',
    current: 'Joriy',
    skillsTitle: "Ko'nikmalar",
    skillsLang: 'Deutsch',
    battleTitle: 'Jang',
    battleWins: 'Yutuqlar',
    battleGames: 'Janglar',
    battleLosses: "Mag'lubiyat",
    vocabTitle: "Mening lug'atim",
    vocabWords: (n: number) => `${n} so'z yodlandi`,
    zoomTitle: 'Zoom tadbirlari',
    zoomSubtitle: '2 ta yaqin tadbir',
    zoomEvent: 'Speaking Club · Bugun 18:00',
    leaderboardTitle: 'Peshqadamlar jadvali',
    leaderboardSubtitle: "Bu hafta · 2-o'rin",
    todayLessons: 'Bugungi darslar',
    fullSchedule: 'Butun jadval',
    noLessonsToday: "Bugun dars yo'q",
    myGroups: 'Guruhlarim',
    noActiveGroups: "Faol guruh yo'q",
    qrCheckIn: 'QR bilan davomatga belgilash',
  },
  schedule: {
    empty: "Jadval bo'sh",
    noLesson: "Dars yo'q",
    today: 'Bugun',
    weekdays: {
      monday: 'Dushanba',
      tuesday: 'Seshanba',
      wednesday: 'Chorshanba',
      thursday: 'Payshanba',
      friday: 'Juma',
      saturday: 'Shanba',
      sunday: 'Yakshanba',
    },
  },
  attendance: {
    total: 'Umumiy davomat',
    present: 'Keldi',
    late: 'Kechikdi',
    absent: 'Kelmadi',
    excused: 'Sababli',
    noRecords: "Yozuv yo'q",
  },
  payments: {
    addBalance: "Balansni to'ldirish",
    amountPlaceholder: "Summa (so'm)",
    afterPaymentNote: "To'lovdan keyin balans bir necha soniyada yangilanadi.",
    history: "To'lovlar tarixi",
    noPayments: "To'lovlar yo'q",
  },
  profile: {
    changePhoto: "Rasmni o'zgartirish",
    uploading: 'Yuklanmoqda...',
    phone: 'Telefon',
    login: 'Login',
    telegram: 'Telegram',
    branch: 'Filial',
    photoSheetTitle: 'Profil rasmi',
    pickGallery: 'Galereyadan tanlash',
    pickCamera: 'Kameradan olish',
    deletePhoto: "Rasmni o'chirish",
  },
  more: {
    profileFallback: 'Profil',
    viewProfile: "Profilni ko'rish",
    payments: "To'lovlar",
    faq: 'FAQ',
    about: 'Biz haqimizda',
    settings: 'Sozlamalar',
    logout: 'Chiqish',
    logoutConfirmTitle: 'Chiqish',
    logoutConfirmMessage: 'Hisobdan chiqishni tasdiqlaysizmi?',
  },
  settings: {
    theme: 'Mavzu',
    themeSystem: 'Tizim',
    themeLight: "Yorug'",
    themeDark: "Qorong'i",
    other: 'Boshqa',
    language: 'Til',
    translator: 'Tarjimon',
  },
  scan: {
    permissionTitle: 'Kamera ruxsati kerak',
    permissionDesc: 'QR kodni skanerlash uchun kameraga ruxsat bering.',
    allow: 'Ruxsat berish',
    guide: 'Dars QR kodiga qarating',
    insufficientTitle: 'Balans yetarli emas',
    insufficientMessage: 'Dars uchun balansingiz yetmadi',
    markedTitle: 'Belgilandi ✓',
    markedMessage: 'Davomat muvaffaqiyatli belgilandi',
    invalidQr: 'QR kod yaroqsiz yoki muddati tugagan',
  },
  about: {
    centerName: 'DAF Sprachzentrum',
    tagline: "Nemis tili o'quv markazi",
    aboutTitle: 'Markaz haqida',
    aboutBody:
      "DAF Sprachzentrum — nemis tilini zamonaviy uslublarda o'rgatuvchi markaz. Ushbu ilova orqali jadvalingizni, davomatingizni va to'lovlaringizni bir joyda kuzatib borasiz.",
    contact: 'Aloqa',
    version: (v: string) => `Ilova versiyasi ${v}`,
  },
  faq: [
    {
      q: "Balansni qanday to'ldiraman?",
      a: "Ko'proq → To'lovlar bo'limiga kiring, summani tanlang va Payme yoki Click orqali to'lang. Balans bir necha soniyada yangilanadi.",
    },
    {
      q: 'Davomat qanday belgilanadi?',
      a: "Darsga kelganingizda Asosiy sahifadagi «QR bilan belgilash» tugmasini bosib, dars QR kodini skanerlang.",
    },
    {
      q: "Balansim manfiy bo'lsa nima bo'ladi?",
      a: "Manfiy balans qarzdorlikni bildiradi. Darslarni davom ettirish uchun iltimos balansni to'ldiring.",
    },
    {
      q: 'Jadvalim va davomatim qayerda?',
      a: "Asosiy sahifada balans, bugungi darslar va davomat ko'rsatkichlari jamlangan.",
    },
    {
      q: "Profil rasmini qanday o'zgartiraman?",
      a: "Ko'proq → Profil bo'limiga kiring, rasm ustiga bosing va galereya yoki kameradan rasm tanlang.",
    },
  ],
  // Learning screens (dummy content pending backends). German learning terms
  // (lesson names, "Lektion", "Hörübung", level names, richtig/falsch) stay
  // German in BOTH languages — they are the subject matter, not UI chrome.
  darslar: {
    title: 'Darslar',
    subtitle: 'Deutsch kursi',
    continue: 'Davom ettirish',
    continueProgress: (p: number, units: number, done: number) =>
      `${p}% tugatildi · ${units} bo'limdan ${done} tasi`,
    statStreak: 'Kun seriya',
    statDone: 'Tugatilgan',
    statPoints: 'Ballar',
    badgeDone: 'Tugatildi',
    badgeActive: 'Davom etmoqda',
    badgeLocked: 'Yopiq',
    unitsWord: (n: number) => `${n} dars`,
    minutesWord: (n: number) => `${n} daqiqa`,
    percentDone: (p: number) => `${p}% tugatildi`,
    lessonsCount: (done: number, total: number) => `${done}/${total} dars`,
    lockedTitle: (level: string) => `${level} hali yopiq`,
    lockedDesc: (prev: string, level: string) =>
      `${prev} darajani to'liq tugatgach, ${level} bosqichi avtomatik ochiladi.`,
    hoerDesc: '4 ta audio · richtig/falsch mashqi',
    lessonSub: {
      l1: 'Tanishuv va salomlashish',
      l2: 'Raqamlar va alifbo',
      l3: "Oila a'zolari",
      l4: 'Kafe va oziq-ovqat',
      l5: 'Kundalik ish-harakatlar',
      l6: 'Xarid qilish va narxlar',
      l7: 'Uy va xonalar',
      l8: "Bo'sh vaqt va sevimli mashg'ulotlar",
    },
  },
  resurslar: {
    title: 'Resurslar',
    subtitle: "O'quv materiallari",
    badgeNew: 'Yangi',
    featuredTitle: "Bu haftaning to'plami",
    featuredDesc: 'A1 · Wortschatz va Grammatika — 12 ta material bir joyda',
    featuredDownload: 'Yuklab olish · 3.5 MB',
    hoerDesc: '4 ta audio · richtig/falsch mashqi',
    materialsCount: (n: number) => `${n} ta material`,
    noMaterials: "Material yo'q",
    noMaterialsDesc: "Bu bo'limda hozircha material mavjud emas.",
    type: { pdf: 'PDF', audio: 'Audio', video: 'Video', book: 'Kitob', exercise: 'Mashq' },
    cat: { all: 'Barchasi', grammar: 'Grammatika', audio: 'Audio', video: 'Video', books: 'Kitoblar' },
    resSub: {
      r1: 'A1 grammatika · der/die/das qoidalari',
      r2: "Tanishuv va o'zini tanishtirish",
      r3: 'Restoranda buyurtma tinglash mashqi',
      r4: "Fe'llarni tuslash · 15 savol",
      r5: "To'liq darslik (PDF)",
      r6: 'Umlaut tovushlari talaffuzi',
      r7: 'Raqamlarni sanash va yozish',
      r8: "500 ta asosiy so'z ro'yxati",
      r9: 'Shifokorda suhbat · audio dialog',
      r10: "Modal fe'llar · 12 mashq",
    },
    resMeta: {
      r1: '2.4 MB', r2: '12:45', r3: '8:20', r4: '15 savol', r5: '18 MB',
      r6: '5:10', r7: '9:30', r8: '0.9 MB', r9: '6:40', r10: '12 mashq',
    },
  },
  vocabulary: {
    newWords: "Yangi so'zlar",
    allWords: "Barcha so'zlar",
    learningWords: "O'rganilayotgan so'zlar",
    period: { '7 kun': '7 kun', '1 oy': '1 oy', '6 oy': '6 oy', '1 yil': '1 yil' },
    weekLabels: ['1-h', '2-h', '3-h', '4-h'],
    weekday: ['Yak', 'Du', 'Se', 'Chor', 'Pay', 'Ju', 'Shan'],
    month: ['Yan', 'Fev', 'Mar', 'Apr', 'May', 'Iyn', 'Iyl', 'Avg', 'Sen', 'Okt', 'Noy', 'Dek'],
  },
  battle: {
    headerStart: 'Jangni boshlang',
    chooseType: 'Jang turlarini tanlang:',
    chooseLobby: 'Lobby turlarini tanlang:',
    start: 'Jangni boshlang',
    aiTitle: 'AI ga Qarshi',
    aiSub: "Sun'iy intellekt bilan bellashing",
    duelTitle: 'Duel',
    duelSub: 'Bir-biriga qarshi jang',
    groupTitle: "Guruhli o'yin",
    groupSub: "4 yoki undan ortiq o'yinchi bilan o'ynang",
  },
  battlePlay: {
    waitingHeader: 'Jangni kutish',
    startsIn: 'Jang boshlanadi:',
    resultHeader: 'Jang natijasi',
    winner: "G'olib!",
    secondPlace: "2-o'rin",
    yourAnswers: 'Sizning javoblaringiz:',
    replay: "Qayta o'ynash",
    goBack: 'Ortga qaytish',
    buildPrompt: "So'zlardan gap yoki savol tuzing",
    check: 'Tekshirish',
    correct: "To'g'ri",
    wrong: 'Xato',
    continue: 'Davom etish',
    yourAnswer: 'Sizning javobingiz:',
    correctAnswer: "To'g'ri javob:",
    timedOut: 'Vaqt tugadi',
    noAnswer: "(javob yo'q)",
    you: 'Siz',
    hints: [
      'Ismingiz nima?',
      'Mening ismim Barno',
      'Men talabaman',
      'Salom, men Ahmadman',
      "Men O'zbekistondanman",
      "U har kuni o'qiydi",
    ],
  },
  hoeren: {
    restart: 'Qaytadan boshlash',
    goBack: 'Ortga qaytish',
    finish: 'Yakunlash',
    correctPct: (pct: number) => `${pct}% to'g'ri javob`,
    instruction:
      "Audioni tinglang va gap to'g'ri (richtig) yoki noto'g'ri (falsch) ekanini tanlang.",
    hints: [
      "Ayol do'kondan non va sut sotib oladi.",
      'Mijoz soat 16:30 dagi tomosha uchun chipta sotib oladi.',
      'Bugun ob-havo quyoshli va issiq.',
      "Siz do'stingizga qo'ng'iroq qilishingiz kerak.",
    ],
  },
  leaderboard: {
    thisWeek: 'Bu hafta',
    you: 'Siz',
  },
};

/** The canonical string shape every language file must satisfy. */
export type Dict = typeof uz;
