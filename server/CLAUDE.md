...
- **A student card is born with its sign-in account.** Admin create (`createStudentUser`), mock participant conversion (in the card's own transaction) and Telegram registration (still inline, same rules) all open it through `openStudentAccount`'s rules; conversion also refuses a phone that is already on a live card, like admin create. A new file that writes Student rows fails `common/auth/student-account.single-source.spec.ts` until it opens the account too. No path shows the generated password: the student gets one from the bot's "Parolni tiklash" or signs in with Telegram.
