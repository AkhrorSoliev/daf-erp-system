...
- **A student signs in with the number on their card (ADR-0032).** Every lookup above reads the ACCOUNT (`User.login` / `User.phone`), staff edit the CARD (`Student.phone`). `StudentsWriteService.update` therefore moves the account in the same transaction as the card, through `planPhoneChange(..., { staff: false })`: `phone` follows, and a `login` holding the old number follows too — or becomes `null` when the new number is already a live login. It compares against the account, not the card's previous value, so the next save of a card edited before the rule heals it. No other code may write an existing card's phone: `students/student-phone.single-source.spec.ts` fails the build. Accounts left behind before the rule: `scripts/repair-student-sign-in-number.ts` (dry run by default).
...
- `StatusCascadeService.cascadeEnrollmentStatus()` — helper used by all cascade transitions (Branch/Course/Group/Student status changes that flip enrollments), and by group deletion (`GroupsWriteService.delete()` → `cascadeGroupDeletion()`), which passes its own transaction so the log rows commit with the deletion
...
- Group deletion: the same refund for every live enrollment of the group, inside the deletion's own transaction — one failed refund rolls the whole deletion back. The status-change cascades (branch close, group cancel, student expel) instead refund each enrollment in its own transaction and log a failure, so one bad enrollment does not hold up a batch of hundreds.
