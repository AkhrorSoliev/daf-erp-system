...
- **`startEmployeeRegistration` releases `ctx.session.processing` in a `finally`.** Telegraf saves the session even when a handler throws, and the `/start` middleware ignores a chat whose flag is set, re-saving it so the 24-hour TTL restarts on every try: a flag left set by a throw (a database error, a missing `TELEGRAM_LINK_SECRET`) locks that person out of `/start` for as long as they keep trying. The error still propagates to Telegraf's handler and is logged. **Known gap:** the student-group, mock-exam and student branches of `startFlow` still reset the flag by hand and do not survive a throw.
