...
@Injectable()
export class EnrollmentTransferReasonsService {
  constructor(
  ...
  findAll(companyId: number) {
    return this.prisma.enrollmentTransferReason.findMany({
  ...
  async create(
    dto: CreateEnrollmentTransferReasonDto,
    companyId: number,
    userId: number,
  ) {
    ...
    const existing = await this.prisma.enrollmentTransferReason.findFirst({
    ...
    const created = await this.prisma.enrollmentTransferReason.create({
      data: { name, companyId },
    });
  ...
  async update(
    id: string,
    dto: UpdateEnrollmentTransferReasonDto,
    companyId: number,
    userId: number,
  ) {
    const existing = await this.prisma.enrollmentTransferReason.findFirst({
    ...
    if (dto.name !== undefined) {
      ...
      if (name !== existing.name) {
        const clash = await this.prisma.enrollmentTransferReason.findFirst({
    ...
    const updated = await this.prisma.enrollmentTransferReason.update({
      where: { id },
  ...
  async remove(id: string, companyId: number, userId: number) {
    ...
    await this.prisma.enrollmentTransferReason.update({
      where: { id },
