...
function makeService(overrides: Record<string, any> = {}) {
  ...
  const authService = {
    findAccountByIdentifier: jest.fn().mockResolvedValue({
      id: 5,
      // A real-shaped bcrypt hash — `findAccountByIdentifier` returns the RAW
      // row (password included). Without a non-empty value here,
      // `JSON.stringify` would silently drop an `undefined` password key and
      // the `completeHandoff` `toEqual` assertion below would pass even if a
      // future refactor leaked the raw row into the handoff payload — this
      // field is what turns that assertion into a real lock.
...
describe('TelegramOauthService', () => {
  ...
  describe('handleCallback', () => {
    ...
    it('parol o\'rnatilmagan akkauntni rad etadi (Telegram yo\'li parol yo\'lidan kengroq bo\'lmasin)', async () => {
      ...
      // findAccountByIdentifier orqali xuddi shu RAW qatorni oladi, shuning
      // uchun xuddi shu tekshiruvni takrorlashi shart.
      mockTokenEndpoint({ id_token: 'signed.id.token' });
      const { service, authService, redis } = makeService();
      authService.findAccountByIdentifier.mockResolvedValue({
        id: 5,
        password: null,
        roles: [{ role: { id: 3, name: 'Administrator' } }],
      });
