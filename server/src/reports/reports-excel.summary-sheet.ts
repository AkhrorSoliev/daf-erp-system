...
function buildBlock1(
  ...
  compareRow(
    ...
    cur.np.teacherSalary,
    prev.np.teacherSalary,
    'Shu oy darslari uchun ustozlar haq qilgan oylik. Naqd keyingi oy chiqadi.',
    { inverse: true },
  );
  compareRow(ws, "o'quvchilar to'lagan qismi", cur.covered, prev.covered, '', {
    sub: true,
  });
  compareRow(
    ws,
    ...
    cur.centerFunded,
    prev.centerFunded,
    "Qarzdor o'quvchi darsi uchun markaz o'z hisobidan qoplagani. Yuqoridagi summa ICHIDA.",
    { sub: true },
  );
  ...
  compareRow(
    ws,
    '−  Xarajatlar (ijara, marketing, kommunal…)',
    cur.np.operatingExpenses,
    prev.np.operatingExpenses,
    'Ustoz avansi bu yerda EMAS — u oylik ichida.',
    { inverse: true },
  );
  compareRow(
    ws,
...
function buildBlock2(
  ...
  compareRow(ws, '−  Qaytarilgan', cur.np.refunds, prev.np.refunds, '', {
    inverse: true,
  });
  headlineRow(
    ws,
    `=  ${curLabel.toUpperCase()}NING O'Z FOYDASI`,
    ownProfit.cur,
    ownProfit.prev,
    ownProfit.cur >= 0
...
function buildBlock3(
  ...
  pctRow(
    ...
    cashIn,
    'Shu oy darslari uchun kelgan pul.',
  );
  for (const l of attribution.late) {
    pctRow(
      ...
      l.amount,
      cashIn,
      'Eski qarzning undirilishi.',
    );
  }
  totalsBar(ws, ['Jami tushgan pul', cashIn, 100, '', ''], [2]).getCell(
    3,
  ).numFmt = PCT;
  const info = ws.addRow([
    `${paymentCount} ta to'lov · ${payerCount} ta o'quvchi`,
    '',
    '',
    '',
...
function buildBlock4(
  ...
  pctRow(
    ws,
    `${curLabel} ichida to'langan`,
    lessonMoney.paidInMonth,
    total,
    '',
  );
  pctRow(
    ws,
    `${curLabel}dan oldin to'langan (balansdagi pul)`,
    lessonMoney.paidEarlier,
    total,
    "O'quvchi avvalroq to'lab qo'ygan, puli balansida turgan edi — dars o'tilganda o'shandan yechilgan.",
  );
  pctRow(
    ws,
    `${nextMonthLabel}da to'langan (kechikkan)`,
    lessonMoney.paidNextMonth,
    total,
...
function buildBlock5(ws: Worksheet, input: SummaryInput): void {
  ...
  for (const { label, amount } of [...cashOut].sort(
...
function buildBlock6(
  ...
  const statusCount = (status: string) =>
    students.byStatus.find((s) => s.status === status)?.count ?? 0;
  ...
  countRow(
    ...
    students.attended,
    "Shu oy kamida bir marta darsga kelgan o'quvchilar — eng haqiqiy 'faol' raqam.",
    { bold: true },
  );
  countRow(
    ws,
  ...
  countRow(
    ...
    students.groupless,
    "DIQQAT: bular 'faol' sanaladi, lekin hech qaysi guruhda yo'q. Ish talab qiladi.",
    { bad: true },
  );
  ws.addRow([]);
  ...
  netRow.getCell(2).font = {
    bold: true,
    size: 12,
    color: { argb: students.netChange >= 0 ? GREEN : RED },
  };
  izohCellFor(netRow);
}
