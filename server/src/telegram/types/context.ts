...
export interface SessionData extends Scenes.SceneSession {
   ...
   * alongside the first. Whoever sets it releases it in a `finally`
   * (`withProcessingLock` does both). Telegraf saves the session even when a
   * handler throws, and every `/start` ignored saves it again, restarting its
   * 24-hour expiry, so a flag left set by a throw shuts the person out of the
   * bot for as long as they keep trying.
   */
  processing?: boolean;
