...
async function main() {
  console.log('Lesson date period (UTC):',
    LESSON_DATE_START.toISOString(), '→', LESSON_DATE_END.toISOString());
  ...
  for (const t of teachers) {
    const groupMap = new Map(
      t.groupTeachers.map((gt) => [gt.group.id, gt.group]),
    );
    ...
    if (groupIds.length > 0) {
      ...
      const consumptionTxs = attendanceIds.length === 0 ? [] : await prisma.transaction.findMany({
        where: {
      ...
      const consumptionByAttId = new Map<string, { perLessonCost: number; paidOn: Date }>();
    ...
    const percentageConfig = t.salaryConfigs.find(
    ...
    const calculatedSalary = percentage != null
      ? Math.round((paidAmount * percentage) / 100)
      : 0;
  ...
  summary.columns = [
    ...
    { header: "Ustoz o'qitayotgan guruhlar soni", key: 'groupCount', width: 22 },
    { header: "May oyida o'tilgan va to'langan darslari bo'lgan o'quvchilar soni", key: 'paidStudentsCount', width: 36 },
    { header: "May oyida o'tilgan to'langan darslar soni", key: 'paidLessonsCount', width: 30 },
    { header: "May oyida o'tilgan qarz (to'lanmagan) darslar soni", key: 'unpaidLessonsCount', width: 32 },
    { header: "May darslari uchun haqiqatda to'langan summa (so'mda)", key: 'paidAmount', width: 40 },
    { header: "Ustozning oylik foizi (%)", key: 'percentage', width: 22 },
    { header: "Ustozga to'lash uchun summa (to'langan dars × foiz, so'mda)", key: 'calculatedSalary', width: 42 },
  ];
  rows.forEach((r, i) => summary.addRow({
    idx: i + 1,
  ...
  for (const r of rows) for (const d of r.details) if (d.isPaid) uniqPayingStudents.add(d.studentId);
  ...
  for (const r of rows) {
    let base = sanitizeSheetName(`${r.teacherName}`);
    let name = base;
    let suffix = 2;
    ...
    sh.columns = [
      ...
      { header: `Ustozning ulushi (foiz ${teacherFoiz}%, so'mda)`, key: 'share', width: 28 },
    ];
    ...
    if (r.details.length === 0) {
      sh.addRow({ idx: '', date: '', group: '', student: "Ushbu ustozning May oyida o'tilgan darsi yo'q", studentId: '', cost: '', status: '', paidOn: '', share: '' });
    } else {
      ...
      for (const groupName of sortedGroups) {
        const items = byGroup.get(groupName)!.slice().sort(
          (a, b) => a.date.getTime() - b.date.getTime() || a.studentId - b.studentId,
        );
        ...
        for (const d of items) {
          const share = r.percentage != null && d.isPaid
        ...
        subRow.eachCell((c) => {
          c.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFBFBE6' } };
        });
        sh.addRow({});
      }
  ...
  console.log('Total paid lessons     :', rows.reduce((s, r) => s + r.paidLessonsCount, 0));
  console.log('Total unpaid lessons   :', rows.reduce((s, r) => s + r.unpaidLessonsCount, 0));
  console.log('Total paid amount      :', rows.reduce((s, r) => s + r.paidAmount, 0).toLocaleString('en-US'));
  console.log('Total unpaid amount    :', rows.reduce((s, r) => s + r.unpaidAmount, 0).toLocaleString('en-US'));
  console.log('Total salary to pay    :', rows.reduce((s, r) => s + r.calculatedSalary, 0).toLocaleString('en-US'));
