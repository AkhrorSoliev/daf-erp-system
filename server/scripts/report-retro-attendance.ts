...
// Configurable so the script isn't pinned to one developer's laptop. Defaults
// to the OS temp dir; override with OUT_PATH=/some/where.csv.
const OUT =
  process.env.OUT_PATH ??
  path.join(os.tmpdir(), 'phantom-attendance-report.csv');
...
(async () => {
  ...
  const flagged = atts.filter(
    (a) => gap(a.date, a.createdAt) >= GAP_DAYS && !TEST_IDS.has(a.studentId),
  );
  ...
  for (const t of tAtts)
    if (!firstTeacher.has(t.studentId)) firstTeacher.set(t.studentId, t.date);
  ...
  for (const f of flagged) {
    ...
    const perLesson = Math.round(
      (f.group.course.price || 0) / (f.group.course.lessonPaymentCount || 12),
    );
    rows.push({
      group: f.group.name,
      teacher:
      ...
      beforeStart: f.group.startDate
        ? f.date.getTime() < f.group.startDate.getTime()
        : false,
    });
  }
  rows.sort((a, b) =>
    a.group < b.group
      ...
      : a.group > b.group
        ? 1
        : a.sid - b.sid || (a.date < b.date ? -1 : 1),
  );

  // CSV
  const header =
    'Guruh,Oqituvchi,Guruh_boshlanishi,Oquvchi_ID,Oquvchi,Balans,Dars_sanasi,Kun,Status,Kim_kiritgan,Qachon_kiritgan,Dars_narxi,Oqituvchi_1belgi,Guruhdan_oldinmi';
  ...
  console.log(
    `Phantom dars qatorlari: ${rows.length} | o'quvchilar: ${new Set(rows.map((r) => r.sid)).size}\n`,
  );
  ...
  for (const g of groupSummary) {
    console.log(`\n▸ ${g.k}`);
    console.log(
      `  ${g.studs} o'quvchi, ${g.lessons} phantom dars, ≈${g.exposure.toLocaleString()} so'm`,
    );
    const perStud = new Map<number, Row[]>();
    for (const r of g.rs)
      (perStud.get(r.sid) ?? perStud.set(r.sid, []).get(r.sid)!).push(r);
  ...
  for (const r of beforeStart)
    console.log(
      `  #${r.sid} ${r.sname} | ${r.group} boshl ${r.groupStart} | dars ${r.date}`,
    );
